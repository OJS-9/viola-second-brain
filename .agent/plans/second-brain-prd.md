...
**Implementation note (built 2026-07-08, 4.1 code-complete/partially live-tested, 4.2 code-complete/live-tested):** `notion-client` 3.1.0 (current PyPI release, added as an explicit dependency via `uv add notion-client`) already defaults `notion_version` to `"2025-09-03"` in its `ClientOptions` dataclass — no manual header pinning needed, just `Client(auth=..., notion_version="2025-09-03")` (passed explicitly anyway for clarity/pinning against future SDK default changes). Confirmed directly from the SDK source (`client.py`/`api_endpoints.py`) and Notion's own docs via a live web search: `databases.retrieve(database_id=...)` returns an object with a `data_sources` array (each entry `{"id", "name"}`) — "The Retrieve Database API is now repurposed to return a list of `data_sources`" per Notion's upgrade guide — confirming the PRD's assumed resolve-then-cache flow was correct. `notion_api.py` resolves the first data source per database (this project only needs single-data-source databases) and caches the mapping in-process. The SDK also ships its own retry logic (429 always retried honoring `Retry-After`; 5xx retried for idempotent GET/DELETE only) layered underneath this project's `shared.with_retry()` — belt-and-suspenders, not a conflict. `nimble_api.py` uses raw `httpx` calls (added as an explicit dependency; it was already a transitive dep via other packages but is now pinned directly since this module imports it) rather than an SDK, matching the PRD's endpoint-level spec exactly (`POST /v1/search`, `POST /v1/extract`, base `https://sdk.nimbleway.com`, Bearer auth). The automatic thin-markdown → `render=True` retry fallback and the 400/402-never-retry rule were both implemented and exercised live. SERP (`/v1/serp`) was **not** built — out of scope for this pass, the brief's "what to build" section didn't list it as a required function, only `search()` and `extract()`; flagged here as a gap vs. the full PRD spec, not a silent drop. **Live test results (2026-07-08):** Nimble tested end-to-end for real — `nimble search "cybersecurity funding news"` returned live results, `nimble extract` succeeded against `example.com` (and correctly auto-retried with `render=True` on the thin "Example Domain" page) and correctly surfaced a real upstream 500 (`server returned 403` from the target site) as a clean error rather than a crash when tested against a live news URL. Notion could **not** be live-tested: `.claude/scripts/.env` does not exist on disk (confirmed via `Path.exists()` immediately before testing) despite Nimble's key resolving successfully — meaning `NIMBLE_API_KEY` is present as a real OS/session environment variable rather than via the `.env` file, while `NOTION_API_KEY` is absent from both. `registry.is_configured("notion")` correctly reports `False` and the CLI prints "set NOTION_API_KEY in .claude/scripts/.env" instead of a stack trace. The Notion module (`whoami`/`tasks`/`create_page`/markdown converter/block-chunking) is code-complete and was verified with unit-level sanity checks (markdown→blocks conversion, 32-hex ID extraction from URLs, 100-block/2000-char chunking math) but never exercised against Notion's live API — genuinely unverified until a real `NOTION_API_KEY` is available.

**Follow-up, same day (2026-07-08):** `.claude/scripts/.env` was created shortly after (briefly misnamed `.env.txt`, fixed once spotted), resolving the blocker above. `notion whoami` authenticated live. Or then explicitly confirmed "Tasks Tracker" as his task database (a unified personal+work tracker, not Viola-specific — task titles seen during testing included academic/coursework items alongside anything work-related, consistent with that framing). `notion tasks --database-id 3414e09e-86f3-80c8-bfd0-f1e0146be352` returned 5 real open tasks with correct structure. Both 4.1 and 4.2 are now code-complete AND live-tested; the Task DB ID is recorded in USER.md, no longer a placeholder.
...
**Implementation note / test result (2026-07-08):** Tested the "known limitation" directly instead of guessing. From this interactive session, `outlook_calendar_search` (query `*`, today's date range) worked immediately and returned a real event — confirming the connector itself is live and reachable. Then ran a genuinely headless test: a one-off script called `claude_agent_sdk.query()` with `ClaudeAgentOptions(setting_sources=["user"], max_turns=3)` — the exact pattern this section specifies, and the same `query()`-based headless pattern `memory_flush.py` already uses in production — asking it to search today's Outlook calendar. Result: the headless session used `ToolSearch` to inspect its own deferred-tool registry and reported back that **no Outlook tool was available at all**; the only calendar connector it could see was Google Calendar. So `setting_sources=["user"]` does not surface the same connector set as an interactive Claude Code session — the Outlook (and by inference Affinity, same OAuth-through-the-Claude-app model) connectors are tied to the interactive/session context, not to user-level settings a headless SDK session can load. **Conclusion: the documented fallback path is now the primary path for 4.3.** Outlook/calendar triage cannot be done by Phase 6's background heartbeat polling — it must be an on-session flow (a `morning-triage` command run when Or opens Claude, or a SessionStart-hook-driven check) per the plan's own contingency. The Notion + Nimble legs of the heartbeat are unaffected (confirmed API-based, no connector dependency). Did not attempt the Microsoft Graph app fallback build — out of scope for this pass and Or hasn't asked for the IT ticket route yet.
