...
3. Run `/create-second-brain-prd ./my-second-brain-requirements.md` in Claude Code
4. You get a 9-phase build plan customized to your platforms, preferences, and infrastructure
...
|-----------|----------|---------|
| **Memory files** | `Dynamous/Memory/` | SOUL.md, USER.md, MEMORY.md, daily logs (synced via Git) |
...
| **Draft management** | `Dynamous/Memory/drafts/` | Active, sent, and expired draft tracking |
| **Habit tracking** | `Dynamous/Memory/HABITS.md` | Recurring habit checklist surfaced during heartbeats |
...
5. Copies starter memory templates to `Dynamous/Memory/` (skips files that already exist)
...
> **Prefer manual setup?** Skip onboarding by deleting `Dynamous/Memory/BOOTSTRAP.md` and editing the memory files in `Dynamous/Memory/` directly.
...
Your Second Brain's personality and knowledge live in `Dynamous/Memory/`. The onboarding conversation fills in the essentials. You can also edit these files directly at any time:
...
mkdir -p ./Dynamous
sudo chown -R 1000:1000 ./Dynamous
...
Then open `https://localhost:8080` in your browser (accept the self-signed certificate), open `/Dynamous` as a vault, and enable Obsidian Sync to connect your remote vault.
...
**Obsidian:** `lscr.io/linuxserver/obsidian`, bound to `127.0.0.1:8080` (mapped from container port 3001), vault mounted at `./Dynamous`. Access via SSH tunnel for Obsidian Sync setup. Timezone defaults to `America/Chicago` — override with `TZ=your/timezone` in your shell environment before running `docker compose up`.
...
Check the daily log (`Dynamous/Memory/daily/YYYY-MM-DD.md`) for `HEARTBEAT_OK` or error entries to confirm whether runs are happening.
...
Onboarding requires `Dynamous/Memory/BOOTSTRAP.md` to exist. If it's missing, copy it manually:

```bash
cp templates/memory/BOOTSTRAP.md Dynamous/Memory/BOOTSTRAP.md
```
