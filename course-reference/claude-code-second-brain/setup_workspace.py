...
def generate_master_template(root: Path) -> None:
    ...
    lines = [
        "# =============================================================================",
        "# Master Environment File — All variables for the Dynamous workspace",
        "# =============================================================================",
...
def init_memory_templates(root: Path, dry_run: bool = False) -> None:
    """Copy memory templates to Dynamous/Memory/ if files don't already exist."""
    templates_dir = root / "templates" / "memory"
    memory_dir = root / "Dynamous" / "Memory"
    ...
    for filename in TEMPLATE_FILES:
        ...
        if dst.exists():
            ...
            print(f"  COPY  templates/memory/{filename} -> Dynamous/Memory/{filename}")
        else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Set up Dynamous workspace")
    parser.add_argument(
        "--env",
        default="master.env",
