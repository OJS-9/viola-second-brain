...
This project uses an Obsidian vault (`Dynamous/Memory/`) as persistent memory across sessions. These files are auto-loaded into context via the `SessionStart` hook — no manual reading required.
...
- **`SessionStart`** — injects SOUL.md, USER.md, MEMORY.md, and recent daily logs into context at session start
...
- **Daily log:** `Dynamous/Memory/daily/YYYY-MM-DD.md` - heartbeat entries appended here
- **Checklist:** `Dynamous/Memory/HEARTBEAT.md` - defines what to check each heartbeat
...
The Obsidian vault (`Dynamous/`) syncs between machines via Git using [simonthum/git-sync](https://github.com/simonthum/git-sync) (CC0 license) on a 2-minute timer. This replaces Obsidian Sync, which can't detect file changes made by external processes (heartbeat, reflection, Claude Code).

**Location:** `.claude/scripts/` (sync scripts), `Dynamous/` (vault repo)
...
| `Dynamous/.gitignore` | Excludes workspace.json, .trash/, lock files |
| `Dynamous/.gitattributes` | Forces LF line endings + concat-both merge driver for daily logs |

### How It Works

1. OS scheduler triggers `run_vault_sync` every 2 minutes
2. Wrapper runs `git-sync` inside `Dynamous/` (the vault directory)
...
1. Markdown files in `Dynamous/Memory/` are chunked into ~400-token overlapping segments
...
- **Daily log:** `Dynamous/Memory/daily/YYYY-MM-DD.md` - reflection entries appended here
...
python .claude/skills/direct-integrations/scripts/query.py gmail create-draft --from-file "Dynamous/Memory/drafts/active/<draft>.md"
...
All account IDs, project GIDs, and service preferences are in `Dynamous/Memory/USER.md`. Refer there for specific values when running commands.
