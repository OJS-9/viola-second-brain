...
Chunks markdown files under SecondBrain/Memory/ into ~400-token overlapping
segments and indexes them into the local SQLite search index (db.py).
Incremental: only files whose content hash changed since the last run are
re-chunked/re-embedded. Stale index rows (files deleted from the vault) are
swept — see remove_stale_files() for why this is not a "never delete vault
content" violation.
...
Usage:
    ...
    uv run python memory_index.py --test       # Dry run (list files only)
"""
...
def chunk_markdown(
    ...
    Uses a ~4 chars/token heuristic. Tracks section titles (# and ##
    headings) and 1-indexed line numbers for each chunk.
    """
    chars_per_token = 4
...
def list_memory_files(memory_dir: Path) -> list[Path]:
    """Find all .md files recursively in the memory vault."""
...
def index_file(
    ...
    # Old chunks for this file are stale the moment content changed — drop
    # them before inserting the fresh set (see db.delete_chunks_for_file
    # docstring for why this isn't a "never delete" violation).
...
def remove_stale_files(current_paths: set[str]) -> int:
    """Remove index rows for files that no longer exist in the vault.

    This deletes *derived search-index data only* — never the markdown files
    themselves. It exists so the index doesn't accumulate dead entries for
    notes that were moved or archived. It is not covered by the project's
    "never delete anything" rule, which protects vault content the user
    wrote; this cache regenerates from whatever's on disk on the next run.
    """
...
def sync_index(
    ...
    Returns counts: files_total, files_indexed, files_skipped,
    ...
    for file_path in files:
        ...
        chunks_created = index_file(file_path, memory_dir, generate_embeddings)
        total_chunks += chunks_created
        files_indexed += 1

    files_removed = remove_stale_files(current_paths)
