...
spawn. Mirrors pre_compact_flush.py; additionally skips trivial sessions.
"""
...
def main() -> None:
    ...
    # trigger its own SessionEnd hook, causing an infinite spawn loop.
    if os.environ.get("CLAUDE_INVOKED_BY"):
        log_hook_execution("session-end-flush", "unknown", "SKIP", _time.time() - _start, "recursion guard")
        sys.exit(0)
    ...
    try:
        ...
        print(f"[session-end-flush] Failed to write context file: {e}", file=sys.stderr)
        log_hook_execution("session-end-flush", source, "ERROR", _time.time() - _start, f"write: {e}")
        sys.exit(0)
