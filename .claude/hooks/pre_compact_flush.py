...
This hook does NO API calls itself — pure local file I/O, then a detached
spawn. It must exit immediately without waiting for the spawned process.
"""
...
def extract_conversation_context(transcript_path: Path) -> str:
    ...
    # Take last N turns, skipping tool-call-only turns (already filtered above
    # since we only keep blocks with actual text)
...
def main() -> None:
    ...
    # Recursion guard: the Agent SDK's own headless subprocess (spawned inside
    # memory_flush.py's query() call) is itself a Claude Code session and would
    # trigger its own PreCompact hook, causing an infinite spawn loop.
    if os.environ.get("CLAUDE_INVOKED_BY"):
        log_hook_execution("pre-compact-flush", "compact", "SKIP", _time.time() - _start, "recursion guard")
        sys.exit(0)
    ...
    # Write context file for the background process
    ...
    try:
        ...
        print(f"[pre-compact-flush] Failed to write context file: {e}", file=sys.stderr)
        log_hook_execution("pre-compact-flush", "compact", "ERROR", _time.time() - _start, f"write: {e}")
        sys.exit(0)
    ...
    cmd = [
        ...
        str(SCRIPTS_DIR),
        "python",
        str(SCRIPTS_DIR / "memory_flush.py"),
        "--context-file",
        str(context_path),
    ]
